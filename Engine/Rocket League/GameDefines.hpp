...
enum EFunctionFlags : uint64_t {
  ...
  FUNC_FuncOverrideMatch =
      (FUNC_Exec | FUNC_Final | FUNC_Latent | FUNC_PreOperator | FUNC_Iterator |
       FUNC_Static | FUNC_Public | FUNC_Protected | FUNC_Const),
  FUNC_NetFuncFlags =
...
enum EPropertyFlags : uint64_t {
  ...
  CPF_Const = 0x0000000000000002, // Actor's property always matches class's
                                  // default actor property.
  CPF_Input = 0x0000000000000004, // Variable is writable by the input system.
  CPF_ExportObject = 0x0000000000000008, // Object can be exported with actor.
  CPF_OptionalParm =
      0x0000000000000010,       // Optional parameter (if CPF_Param is set).
  ...
  CPF_SkipParm = 0x0000000000000200, // Property is a short-circuitable
                                     // evaluation function parm.
  CPF_ReturnParm = 0x0000000000000400, // Return value.
  CPF_CoerceParm =
      0x0000000000000800,          // Coerce args into this function parameter.
  CPF_Native = 0x0000000000001000, // Property is native: C++ code is
                                   // responsible for serializing it.
  CPF_Transient = 0x0000000000002000, // Property is transient: shouldn't be
                                      // saved, zero-filled at load time.
  CPF_Config = 0x0000000000004000,    // Property should be loaded/saved as
                                      // permanent profile.
  CPF_Localized =
      0x0000000000008000, // Property should be loaded as localizable text.
  ...
  CPF_Component =
      0x0000000000080000, // Property containts component references.
  CPF_AlwaysInit =
      0x0000000000100000, // Property should never be exported as NoInit(@todo -
                          // this doesn't need to be a property flag...only used
                          // during make).
  ...
  CPF_NeedCtorLink =
      0x0000000000400000,              // Fields need construction/destruction.
  CPF_NoExport = 0x0000000000800000,   // Property should not be exported to the
                                       // native class header file.
  ...
  CPF_EditFindable = 0x0000000008000000,  // References are set by clicking on
                                          // actors in the editor viewports.
  CPF_Deprecated = 0x0000000020000000,  // Property is deprecated.  Read it from
                                        // an archive, but don't save it.
  CPF_DataBinding = 0x0000000040000000, // Indicates that this property should
                                        // be exposed to data stores.
  CPF_SerializeText =
      0x0000000080000000, // Native property should be serialized as text
                          // (ImportText, ExportText).
  CPF_RepNotify =
      0x0000000100000000, // Notify actors when a property is replicated.
  CPF_Interp =
      0x0000000200000000, // Interpolatable property for use with matinee.
  CPF_NonTransactional = 0x0000000400000000, // Property isn't transacted.
  CPF_EditorOnly =
      0x0000000800000000, // Property should only be loaded in the editor.
  CPF_NotForConsole =
      0x0000001000000000, // Property should not be loaded on console (or be a
                          // console cooker commandlet).
  CPF_RepRetry =
      0x0000002000000000, // Property replication of this property if it fails
                          // to be fully sent (e.g. object references not yet
                          // available to serialize over the network).
  CPF_PrivateWrite = 0x0000004000000000, // Property is const outside of the
                                         // class it was declared in.
  CPF_ProtectedWrite =
      0x0000008000000000, // Property is const outside of the class it was
                          // declared in and subclasses.
  CPF_ArchetypeProperty =
      0x0000010000000000, // Property should be ignored by archives which have
                          // ArIgnoreArchetypeRef set.
  CPF_EditHide = 0x0000020000000000, // Property should never be shown in a
                                     // properties window.
  CPF_EditTextBox =
      0x0000040000000000, // Property can be edited using a text dialog box.
  CPF_CrossLevelPassive =
      0x0000100000000000, // Property can point across levels, and will be
                          // serialized properly, but assumes it's target exists
                          // in-game (non-editor)
  CPF_CrossLevelActive =
      0x0000200000000000, // Property can point across levels, and will be
                          // serialized properly, and will be updated when the
                          // target is streamed in/out
};
...
enum EObjectFlags : uint64_t {
  ...
  RF_DebugFinishDestroyed =
      0x0000000000000020,                  // For debugging FinishDestroy calls.
  RF_EdSelected = 0x0000000000000040,      // Object is selected in one of the
                                           // editors browser windows.
  RF_ZombieComponent = 0x0000000000000080, // This component's template was
                                           // deleted, so should not be used.
  RF_Protected =
      0x0000000000000100, // Property is protected (may only be accessed from
                          // its owner class or subclasses).
  RF_ClassDefaultObject =
      0x0000000000000200, // this object is its class's default object.
  RF_ArchetypeObject =
      0x0000000000000400, // this object is a template for another object (treat
                          // like a class default object).
  RF_ForceTagExp =
      0x0000000000000800, // Forces this object to be put into the export table
                          // when saving a package regardless of outer.
  RF_TokenStreamAssembled = 0x0000000000001000, // Set if reference token stream
                                                // has already been assembled.
  RF_MisalignedObject =
      0x0000000000002000, // Object's size no longer matches the size of its C++
                          // class (only used during make, for native classes
                          // whose properties have changed).
  RF_RootSet = 0x0000000000004000, // Object will not be garbage collected, even
                                   // if unreferenced.
  RF_BeginDestroyed =
      0x0000000000008000, // BeginDestroy has been called on the object.
  RF_FinishDestroyed =
      0x0000000000010000, // FinishDestroy has been called on the object.
  RF_DebugBeginDestroyed =
      0x0000000000020000, // Whether object is rooted as being part of the root
                          // set (garbage collection).
  RF_MarkedByCooker = 0x0000000000040000, // Marked by content cooker.
  RF_LocalizedResource =
      0x0000000000080000, // Whether resource object is localized.
  RF_InitializedProps = 0x0000000000100000, // whether InitProperties has been
                                            // called on this object
  RF_PendingFieldPatches =
      0x0000000000200000, // @script patcher: indicates that this struct will
                          // receive additional member properties from the
                          // script patcher.
  RF_IsCrossLevelReferenced =
      0x0000000000400000, // This object has been pointed to by a cross-level
                          // reference, and therefore requires additional
                          // cleanup upon deletion.
  RF_Saved =
      0x0000000080000000, // Object has been saved via SavePackage (temporary).
  RF_Transactional = 0x0000000100000000, // Object is transactional.
  RF_Unreachable =
      0x0000000200000000, // Object is not reachable on the object graph.
  ...
  RF_Obsolete =
      0x0000002000000000, // Object marked as obsolete and should be replaced.
  RF_TagGarbage = 0x0000004000000000, // Check during garbage collection.
  RF_DisregardForGC =
      0x0000008000000000, // Object is being disregard for GC as its static and
                          // itself and all references are always loaded.
  RF_PerObjectLocalized =
      0x0000010000000000, // Object is localized by instance name, not by class.
  RF_NeedLoad =
      0x0000020000000000, // During load, indicates object needs loading.
  RF_AsyncLoading =
      0x0000040000000000, // Object is being asynchronously loaded.
  RF_NeedPostLoadSubobjects =
      0x0000080000000000, // During load, indicates that the object still needs
                          // to instance subobjects and fixup serialized
                          // component references.
  RF_Suppress = 0x0000100000000000,   // @warning: Mirrored in UnName.h.
                                      // Suppressed log name.
  ...
  RF_Standalone = 0x0008000000000000, // Keep object around for editing even if
                                      // unreferenced.
  RF_NotForClient =
      0x0010000000000000, // Don't load this object for the game client.
  RF_NotForServer =
      0x0020000000000000, // Don't load this object for the game server.
  ...
  RF_PendingKill =
      0x2000000000000000, // Objects that are pending destruction (invalid for
                          // gameplay but valid objects).
  RF_MarkedByCookerTemp = 0x4000000000000000, // Temporarily marked by content
                                              // cooker (should be cleared).
  RF_CookedStartupObject =
      0x8000000000000000, // This object was cooked into a startup package.
  ...
  RF_Load =
      (RF_ContextFlags | RF_LoadContextFlags | RF_Public | RF_Standalone |
       RF_Native | RF_Obsolete | RF_Protected | RF_Transactional | RF_HasStack |
       RF_PerObjectLocalized | RF_ClassDefaultObject | RF_ArchetypeObject |
  ...
  RF_ScriptMask = (RF_Transactional | RF_Public | RF_Transient |
...
enum EPackageFlags : uint32_t {
  ...
  PKG_Cooked = 0x00000008,   // Whether this package has been cooked for the
                             // target platform.
  PKG_Unsecure = 0x00000010, // Not trusted.
  PKG_SavedWithNewerVersion =
      0x00000020,             // Package was saved with newer version.
  PKG_Need = 0x00008000,      // Client needs to download this package.
  PKG_Compiling = 0x00010000, // package is currently being compiled
  PKG_ContainsMap =
      0x00020000,         // Set if the package contains a ULevel/ UWorld object
  PKG_Trash = 0x00040000, // Set if the package was loaded from the trashcan
  PKG_DisallowLazyLoading = 0x00080000, // Set if the archive serializing this
                                        // package cannot use lazy loading
  PKG_PlayInEditor =
      0x00100000, // Set if the package was created for the purpose of PIE
  PKG_ContainsScript =
      0x00200000, // Package is allowed to contain UClasses and unrealscript
  PKG_ContainsDebugInfo =
      0x00400000, // Package contains debug info (for UDebugger)
  PKG_RequireImportsAlreadyLoaded =
      0x00800000, // Package requires all its imports to already have been
                  // loaded
  PKG_StoreCompressed = 0x02000000, // Package is being stored compressed,
                                    // requires archive support for compression
  PKG_StoreFullyCompressed =
      0x04000000, // Package is serialized normally, and then fully compressed
                  // after (must be decompressed before LoadPackage is called)
  PKG_ContainsFaceFXData =
      0x10000000, // Package contains FaceFX assets and/or animsets
  PKG_NoExportAllowed = 0x20000000, // Package was NOT created by a modder.
                                    // Internal data not for export
  PKG_StrippedSource =
      0x40000000, // Source has been removed to compress the package size
...
enum EClassFlags : uint32_t {
  ...
  CLASS_Abstract =
      0x00000001, // Class is abstract and can't be instantiated directly.
  ...
  CLASS_Transient =
      0x00000008, // This object type can't be saved; null it out at save time.
  CLASS_Parsed = 0x00000010,      // Successfully parsed.
  CLASS_Localized = 0x00000020,   // Class contains localized text.
  CLASS_SafeReplace = 0x00000040, // Objects of this class can be safely
                                  // replaced with default or NULL.
  CLASS_Native = 0x00000080, // Class is a native class - native interfaces will
                             // have CLASS_Native set, but not RF_Native.
  ...
  CLASS_PerObjectConfig =
      0x00000400, // Handle object configuration on a per-object basis, rather
                  // than per-class.
  CLASS_NativeReplication = 0x00000800, // Replication handled in C++.
  CLASS_EditInlineNew =
      0x00001000, // Class can be constructed from editinline New button..
  ...
  CLASS_HasInstancedProps =
      0x00200000, // class contains object properties which are marked
                  // "instanced" (or editinline export).
  CLASS_NeedsDefProps =
      0x00400000, // Class needs its defaultproperties imported.
  CLASS_HasComponents = 0x00800000, // Class has component properties.
  CLASS_Hidden = 0x01000000,        // Don't show this class in the editor class
                                    // browser or edit inline new menus.
  CLASS_Deprecated =
      0x02000000, // Don't save objects of this class when serializing.
  CLASS_HideDropDown =
      0x04000000, // Class not shown in editor drop down for class selection.
  ...
  CLASS_NativeOnly = 0x20000000, // Properties in this class can only be
                                 // accessed from native code.
  CLASS_PerObjectLocalized =
      0x40000000, // Handle object localization on a per-object basis, rather
                  // than per-class.
  CLASS_HasCrossLevelRefs = 0x80000000, // This class has properties that are
                                        // marked with CPF_CrossLevel
  ...
  CLASS_Inherit =
      (CLASS_Transient | CLASS_Config | CLASS_Localized | CLASS_SafeReplace |
       CLASS_PerObjectConfig | CLASS_PerObjectLocalized | CLASS_Placeable |
  ...
  // These flags will be cleared by the compiler when the class is parsed during
  // script compilation.
  CLASS_RecompilerClear = (CLASS_Inherit | CLASS_Abstract | CLASS_NoExport |
                           CLASS_NativeReplication | CLASS_Native),

  // These flags will be inherited from the base class only for non-intrinsic
  // classes.
  CLASS_ScriptInherit =
      (CLASS_Inherit | CLASS_EditInlineNew | CLASS_CollapseCategories),
...
private:
  void ReAllocate(int32_t newArrayMax) {
    ElementPointer newArrayData =
        (ElementPointer)::operator new(newArrayMax * sizeof(ElementType));
...
public:
  TMap()
      : IndirectData(NULL), NumBits(0), MaxBits(0), FirstFreeIndex(0),
        NumFreeIndices(0), InlineHash(0), Hash(nullptr), HashCount(0) {}
...
public:
  ...
  TValue &at(const TKey &key) {
    ...
    // Note: Realistically, you should throw an exception or handle "not found"
...
public:
  uint64_t Flags;
  REGISTER_MEMBER(uint64_t, Flags,
                  EMemberTypes::FNameEntry_Flags) // 0x0000 (0x08)
  int32_t Index;
  REGISTER_MEMBER(int32_t, Index,
                  EMemberTypes::FNameEntry_Index) // 0x0008 (0x04)
  class FNameEntry *HashNext;
  REGISTER_MEMBER(class FNameEntry *, HashNext,
                  EMemberTypes::FNameEntry_HashNext) // 0x000C (0x04)

#ifdef UTF16
  uint8_t UnknownData00[4];
  wchar_t Name[0x400];
  REGISTER_MEMBER(wchar_t, Name, EMemberTypes::FNameEntry_Name) // 0x0010 (0x00)
#else
  char Name[0x400];
  REGISTER_MEMBER(char, Name, EMemberTypes::FNameEntry_Name) // 0x0010 (0x00)
#endif
...
struct FMultiScriptDelegate {
public:
  class UObject *Object;
  struct FName FunctionName;
  class TArray<struct FScriptDelegate> *DelegateArray;

public:
  void Call(void *params);
  template <typename T, typename... Args> void Call(Args &&...args);
};
...
// FPointer
// (0x0000 - 0x0004)
struct alignas(8) FPointer {
  uintptr_t Dummy;

  bool operator==(const FPointer &other) const { return Dummy == other.Dummy; }

  bool operator!=(const FPointer &other) const { return !(*this == other); }
};
...
// Uncommenting this will disabling using the "MinAlignment" field in UStruct,
// it is recommended you keep this commented.
...
public:
  ...
  REGISTER_MEMBER(struct FPointer, VfTableObject,
                  EMemberTypes::UObject_VfTable) // 0x0000 (0x04)
  struct FPointer HashNext;
  REGISTER_MEMBER(struct FPointer, HashNext,
                  EMemberTypes::UObject_HashNext) // 0x0010 (0x04)
  uint64_t ObjectFlags;
  REGISTER_MEMBER(uint64_t, ObjectFlags,
                  EMemberTypes::UObject_ObjectFlags) // 0x0008 (0x08)
  struct FPointer HashOuterNext;
  REGISTER_MEMBER(struct FPointer, HashOuterNext,
                  EMemberTypes::UObject_HashOuterNext) // 0x0014 (0x04)
  struct FPointer StateFrame;
  REGISTER_MEMBER(struct FPointer, StateFrame,
                  EMemberTypes::UObject_StateFrame) // 0x0018 (0x04)
  class UObject *Linker;
  REGISTER_MEMBER(class UObject *, Linker,
                  EMemberTypes::UObject_Linker) // 0x001C (0x04)
  struct FPointer LinkerIndex;
  REGISTER_MEMBER(struct FPointer, LinkerIndex,
                  EMemberTypes::UObject_LinkerIndex) // 0x0020 (0x04)
  int32_t ObjectInternalInteger;
  REGISTER_MEMBER(int32_t, ObjectInternalInteger,
                  EMemberTypes::UObject_Integer) // 0x0004 (0x04)
  int32_t NetIndex;
  REGISTER_MEMBER(int32_t, NetIndex,
                  EMemberTypes::UObject_NetIndex) // 0x0024 (0x04)
  class UObject *Outer;
  REGISTER_MEMBER(class UObject *, Outer,
                  EMemberTypes::UObject_Outer) // 0x0024 (0x04)
  class FName Name;
  REGISTER_MEMBER(class FName, Name,
                  EMemberTypes::UObject_Name) // 0x0028 (0x08)
  class UClass *Class;
  REGISTER_MEMBER(class UClass *, Class,
                  EMemberTypes::UObject_Class) // 0x0030 (0x04)
  class UObject *ObjectArchetype;
  REGISTER_MEMBER(class UObject *, ObjectArchetype,
                  EMemberTypes::UObject_Archetype) // 0x0034 (0x04)
...
public:
  class UField *Next;
  REGISTER_MEMBER(class UField *, Next,
                  EMemberTypes::UField_Next) // 0x0038 (0x04)
  uint64_t UniqueFeatureValue;
  REGISTER_MEMBER(uint64_t, UniqueFeatureValue,
...
public:
  class TArray<class FName> Names;
  REGISTER_MEMBER(class TArray<class FName>, Names,
                  EMemberTypes::UEnum_Names) // 0x003C (0x0C)
...
public:
  class FString Value;
  REGISTER_MEMBER(class FString, Value,
                  EMemberTypes::UConst_Value) // 0x003C (0x0C)
...
public:
  int32_t ArraySize;
  REGISTER_MEMBER(int32_t, ArraySize,
                  EMemberTypes::UProperty_ArrayDim) // 0x003C (0x04)
  int32_t PropertySize;
  REGISTER_MEMBER(int32_t, PropertySize,
                  EMemberTypes::UProperty_PropertySize) // 0x0040 (0x04)
  uint64_t PropertyFlags;
  REGISTER_MEMBER(uint64_t, PropertyFlags,
                  EMemberTypes::UProperty_PropertyFlags) // 0x0044 (0x08)
  uint16_t RepOffset;
  REGISTER_MEMBER(uint16_t, RepOffset, EMemberTypes::UProperty_RepOffset)
  uint16_t RepIndex;
  REGISTER_MEMBER(uint16_t, RepIndex, EMemberTypes::UProperty_RepIndex)
  struct FName Category;
  REGISTER_MEMBER(FName, Category, EMemberTypes::UProperty_Category)
  class UEnum *ArraySizeEnum;
  REGISTER_MEMBER(UEnum *, ArraySizeEnum, EMemberTypes::UProperty_ArraySizeEnum)
  uint32_t Offset;
  REGISTER_MEMBER(uint32_t, Offset, EMemberTypes::UProperty_Offset)
  class UProperty *PropertyLinkNext;
  REGISTER_MEMBER(UProperty *, PropertyLinkNext,
                  EMemberTypes::UProperty_PropertyLinkNext)
  class UProperty *ConstructorLinkNext;
  REGISTER_MEMBER(UProperty *, ConstructorLinkNext,
                  EMemberTypes::UProperty_ConstructorLinkNext)
  class UProperty *NextRef;
  REGISTER_MEMBER(UProperty *, NextRef, EMemberTypes::UProperty_NextRef)
  class FString Alias;
  REGISTER_MEMBER(FString, Alias, EMemberTypes::UProperty_Alias)
...
public:
  ...
  REGISTER_MEMBER(
      class UField *, SuperField,
      EMemberTypes::UStruct_SuperField) // 0x0044 (0x04) [SUPERFIELD CAN EITHER
                                        // BE HERE, OR IN UFIELD DPENDING ON THE
                                        // GAME. COMMENT OUT ACCORDINGLY!]
  class UField *Children;
  REGISTER_MEMBER(class UField *, Children,
                  EMemberTypes::UStruct_Children) // 0x0048 (0x04)
  int32_t PropertySize;
  REGISTER_MEMBER(int32_t, PropertySize,
                  EMemberTypes::UStruct_Size) // 0x004C (0x04)
  uint8_t UnknownData01[0x18];
  int32_t MinAlignment;
  REGISTER_MEMBER(int32_t, MinAlignment,
                  EMemberTypes::UStruct_Alignment) // 0x0064 (0x04)
  uint8_t UnknownData02[0x80];
...
public:
  ...
  REGISTER_MEMBER(uint64_t, FunctionFlags,
                  EMemberTypes::UFunction_Flags) // 0x0080 (0x08)
  uint16_t iNative;
  REGISTER_MEMBER(uint16_t, iNative,
                  EMemberTypes::UFunction_Native) // 0x0088 (0x02)
  uint16_t RepOffset;
  REGISTER_MEMBER(uint16_t, RepOffset,
                  EMemberTypes::UFunction_RepOffset) // 0x008A (0x02)
  struct FName FriendlyName;
  REGISTER_MEMBER(struct FName, FriendlyName,
                  EMemberTypes::UFunction_FriendlyName) // 0x013C (0x08)
  uint8_t OperPrecedence;
  REGISTER_MEMBER(uint8_t, OperPrecedence,
                  EMemberTypes::UFunction_OperPrecedence) // 0x0144 (0x01)
  uint8_t NumParms;
  REGISTER_MEMBER(uint8_t, NumParms,
                  EMemberTypes::UFunction_NumParms) // 0x0145 (0x01)
  uint16_t ParmsSize;
  REGISTER_MEMBER(uint16_t, ParmsSize,
                  EMemberTypes::UFunction_ParmsSize) // 0x0146 (0x02)
  unsigned long ReturnValueOffset;
  REGISTER_MEMBER(unsigned long, ReturnValueOffset,
                  EMemberTypes::UFunction_ReturnValueOffset) // 0x0148 (0x04)
  uint8_t UnknownData00[0xC];                                // 0x014C (0x12)
  FPointer Func;
  REGISTER_MEMBER(FPointer, Func, EMemberTypes::UFunction_Func) // 0x0158 (0x08)
...
public:
  uint8_t UnknownData00[0x60]; // 0x0080 (0x48)
...
public:
  class UFunction *SuccessCallback;
  REGISTER_MEMBER(
      UFunction *, SuccessCallback,
      EMemberTypes::UDelegateProperty_SuccessCallback) // 0x006C (0x04)
  class UFunction *FailureCallback;
  REGISTER_MEMBER(
      UFunction *, FailureCallback,
      EMemberTypes::UDelegateProperty_FailureCallback) // 0x0070 (0x04)
...
// Class Core.Class 952 bytes
// 0x01B4 (0x00C8 - 0x01B4)
class UClass : public UState {
public:
  uint32_t ClassFlags;
  REGISTER_MEMBER(uint32_t, ClassFlags,
                  EMemberTypes::UClass_ClassFlags) // 0x00C8 (0x04)
  uint32_t ClassUnique;
  REGISTER_MEMBER(uint32_t, ClassUnique, EMemberTypes::UClass_ClassUnique)
  char ClassDelegates[80];
  REGISTER_MEMBER(ClassDelegates, ClassDelegates,
                  EMemberTypes::UClass_ClassDelegates)
  uint32_t ClassCastFlags;
  REGISTER_MEMBER(uint32_t, ClassCastFlags, EMemberTypes::UClass_ClassCastFlags)
  UClass *ClassWithin;
  REGISTER_MEMBER(UClass *, ClassWithin, EMemberTypes::UClass_ClassWithin)
  FName Category;
  REGISTER_MEMBER(FName, Category, EMemberTypes::UClass_Category)
  TArray<FRepRecord> ClassReps;
  REGISTER_MEMBER(TArray<FRepRecord>, ClassReps, EMemberTypes::UClass_ClassReps)
  TArray<UField *> NetFields;
  REGISTER_MEMBER(TArray<UField *>, NetFields, EMemberTypes::UClass_NetFields)
  TArray<FName> HideCategories;
  REGISTER_MEMBER(TArray<FName>, HideCategories,
                  EMemberTypes::UClass_HideCategories)
  TArray<FName> AutoExpandCategories;
  REGISTER_MEMBER(TArray<FName>, AutoExpandCategories,
                  EMemberTypes::UClass_AutoExpandCategories)
  TArray<FName> AutoCollapseCategories;
  REGISTER_MEMBER(TArray<FName>, AutoCollapseCategories,
                  EMemberTypes::UClass_AutoCollapseCategories)
  TArray<FName> DontSortCategories;
  REGISTER_MEMBER(TArray<FName>, DontSortCategories,
                  EMemberTypes::UClass_DontSortCategories)
  TArray<FName> DependentOn;
  REGISTER_MEMBER(TArray<FName>, DependentOn, EMemberTypes::UClass_DependentOn)
  TArray<FName> ClassGroupNames;
  REGISTER_MEMBER(TArray<FName>, ClassGroupNames,
                  EMemberTypes::UClass_ClassGroupNames)
  uint32_t bForceScriptOrder;
  REGISTER_MEMBER(uint32_t, bForceScriptOrder,
                  EMemberTypes::UClass_bForceScriptOrder)
  FString ClassHeaderFilename;
  REGISTER_MEMBER(FString, ClassHeaderFilename,
                  EMemberTypes::UClass_ClassHeaderFilename)
  uint8_t ManualPad[40];
  REGISTER_MEMBER(ManualPad, ManualPad, EMemberTypes::UClass_ManualPad)
  UObject *ClassDefaultObject;
  REGISTER_MEMBER(UObject *, ClassDefaultObject,
                  EMemberTypes::UClass_ClassDefaultObject)
  void *ClassConstructor;
  REGISTER_MEMBER(void *, ClassConstructor,
                  EMemberTypes::UClass_ClassConstructor)
  void *ClassStaticConstructor;
  REGISTER_MEMBER(void *, ClassStaticConstructor,
                  EMemberTypes::UClass_ClassStaticConstructor)
  void *ClassStaticInitializer;
  REGISTER_MEMBER(void *, ClassStaticInitializer,
                  EMemberTypes::UClass_ClassStaticInitializer)
  char ComponentNameToDefaultObjectMap[80];
  REGISTER_MEMBER(ComponentNameToDefaultObjectMap,
                  ComponentNameToDefaultObjectMap,
                  EMemberTypes::UClass_ComponentNameToDefaultObjectMap)
  TArray<FImplementedInterface> Interfaces;
  REGISTER_MEMBER(TArray<FImplementedInterface>, Interfaces,
                  EMemberTypes::UClass_Interfaces)
  FString DefaultPropText;
  REGISTER_MEMBER(FString, DefaultPropText,
                  EMemberTypes::UClass_DefaultPropText)
  uint32_t bNeedsPropertiesLinked;
  REGISTER_MEMBER(uint32_t, bNeedsPropertiesLinked,
                  EMemberTypes::UClass_bNeedsPropertiesLinked)
  TArray<uint32_t> ReferenceTokenStream;
  REGISTER_MEMBER(TArray<uint32_t>, ReferenceTokenStream,
                  EMemberTypes::UClass_ReferenceTokenStream)
  char ClassStates[80];
  REGISTER_MEMBER(ClassStates, ClassStates, EMemberTypes::UClass_ClassStates)
...
public:
  ...
  REGISTER_MEMBER(class UStruct *, Struct,
                  EMemberTypes::UStructProperty_Struct) // 0x006C (0x04)
...
public:
  ...
  REGISTER_MEMBER(class UClass *, PropertyClass,
                  EMemberTypes::UObjectProperty_Class) // 0x006C (0x04)
...
public:
  ...
  REGISTER_MEMBER(class UClass *, MetaClass,
                  EMemberTypes::UClassProperty_Meta) // 0x0070 (0x04)
...
public:
  class UProperty *Key;
  REGISTER_MEMBER(class UProperty *, Key,
                  EMemberTypes::UMapProperty_Key) // 0x006C (0x04)
  class UProperty *Value;
  REGISTER_MEMBER(class UProperty *, Value,
                  EMemberTypes::UMapProperty_Value) // 0x0070 (0x04)
...
public:
  ...
  REGISTER_MEMBER(class UClass *, InterfaceClass,
                  EMemberTypes::UInterfaceProperty_Class) // 0x006C (0x04)
  uint8_t UnknownData00[0x08];
...
public:
  class UEnum *Enum;
  REGISTER_MEMBER(class UEnum *, Enum,
                  EMemberTypes::UByteProperty_Enum) // 0x006C (0x08)
...
public:
  uint64_t BitMask;
  REGISTER_MEMBER(uint64_t, BitMask,
                  EMemberTypes::UBoolProperty_BitMask) // 0x006C (0x04)
...
public:
  ...
  REGISTER_MEMBER(class UProperty *, Inner,
                  EMemberTypes::UArrayProperty_Inner) // 0x006C (0x04)
